...
def call_gemini_with_retry(prompt: str, json_mode: bool = False) -> str:
    ...
    raise HTTPException(status_code=429, detail=f"Rate limit exceeded: {last_err}")
...
def process_chunks_sequentially(chunks: List[str], base_prompt: str) -> List[Dict[str, Any]]:
    results = []
...
@app.post("/api/decode", response_model=ExtractionResponse)
async def decode_document(compliance_name: str = Form(...), file: UploadFile = File(...)):
    file_bytes = await file.read()
    ver_text = extract_pdf_text(file_bytes, first_pages_only=True)
    ver_prompt = f"Does this match '{compliance_name}'? Return JSON {{'is_match': true}}. Text: {ver_text[:2000]}"
    ver_raw = await asyncio.to_thread(call_gemini_with_retry, ver_prompt, False)
    if not safe_json_parse(ver_raw).get("is_match"):
        raise HTTPException(status_code=400, detail="Verification Failed.")

    chunks = chunk_text(extract_pdf_text(file_bytes))
    base_prompt = "Extract compliance hierarchy to JSON (domains > sub-domains)."
    merged = await asyncio.to_thread(process_chunks_sequentially, chunks, base_prompt)
    return ExtractionResponse(status="success", filename=file.filename, framework_data=merge_framework_chunks(merged))
