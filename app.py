...
.chapter-block { background:#fff; border-radius:18px; padding:24px 28px; margin-bottom:24px; border:1.5px solid #eee6fb; box-shadow:0 4px 16px rgba(138,43,226,0.06); }
.chapter-label { display:inline-block; background:#f3e8ff; color:#6a1cb0; font-weight:700; font-size:0.78rem; padding:3px 12px; border-radius:20px; margin-bottom:10px; letter-spacing:0.04em; text-transform:uppercase; }
.chapter-title { color:#3a0066; font-weight:800; font-size:1.15rem; margin:0 0 16px 0; }

.article-block { border-left:4px solid #8a2be2; padding:14px 18px; margin-bottom:14px; border-radius:0 12px 12px 0; background:linear-gradient(135deg,#faf8ff 0%,#f5f2fc 100%); }
...
.control-block { background:#fff; border:1px solid #e4d9f7; border-radius:10px; padding:12px 16px; margin:8px 0; }
...
.action-item { display:inline-block; background:#f3e8ff; color:#6a1cb0; font-size:0.8rem; padding:3px 10px; border-radius:20px; margin:2px 4px 2px 0; font-weight:500; }

.quota-notice { background:#fff7ed; border:1.5px solid #fed7aa; border-radius:14px; padding:20px 24px; color:#9a3412; font-weight:600; text-align:center; }
.success-banner { background:linear-gradient(135deg,#f3e5f5 0%,#ede7f6 100%); border:1.5px solid #ce93d8; border-radius:16px; padding:32px; text-align:center; margin:20px 0; }
...
# --- Baki code (Session, Sidebar, Hero, Extraction, UI) as it is ---
...
with st.sidebar:
    ...
    if ok:
        ...
        st.session_state.update(initialized=True, company=company, email=usr_email, lang=lang)
        st.rerun()

st.markdown("""<div class="hero-banner"><h1>⚖️ Sahl Compliance Decoder</h1><p>Enterprise GRC Automation Engine</p></div>""", unsafe_allow_html=True)

if not st.session_state.initialized:
    st.warning("Please initialize your workspace in the sidebar to proceed.")
    st.stop()

# [EXTRACTION_PROMPT, run_extraction function, Upload/Results UI logic as per your original file]
# ... [Pasting your existing rest of the code here exactly] ...
# ── Prompt ───────────────────────────────────────────────────────────────
EXTRACTION_PROMPT = """
...
RULES:
- Detect the hierarchy intelligently: GDPR=Chapter/Article, NCNICC/ECC=Domain/Sub-domain/Control,
...
- action_list: suggest 2-3 realistic policy documents or evidence items needed to comply.
        ...
        for ci, ch in enumerate(chapters):
            ...
            for ai, art in enumerate(ch.get("articles", [])):
                with st.expander(f"Article {art.get('article_id','')} — {art.get('article_title_en','')[:60]}"):
                    ...
                    new_articles.append({
                        "article_id": art_id, "article_title_en": art_en,
                        "article_title_ar": art_ar, "description_en": desc_en,
                        "description_ar": desc_ar,  "controls": new_controls
                    })
            new_chapters.append({
